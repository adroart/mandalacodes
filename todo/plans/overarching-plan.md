...
- 2026-09-08 · WordForge loop landed (i64os PR 144, merged). Click path: WordForge, the Oracle card template, Series view, a card, a lens. The brief sits above the editor (frontmatter, the arc, the four-lens question, hard rules, banned words), the preview renders beside it as you type, Propose prepares a commit, Open draft PR pushes it as a draft against mandalacodes. Seen in screenshots: no icons, no italics, no em-dashes. Fault seen: the editor is a narrow box inside the library side sheet with its label under it; not yet a room to write in. Follow-up dispatched: a full-width writing pane. A3 (briefs and tracker) is covered by this surface plus the series grid; no separate markdown briefs will be written.
- 2026-09-08 · WordForge writing pane landed (i64os PR 147, merged). The editor is now the primary surface: full width, serif, label above, preview beside at roughly 62/38, brief collapsed to frontmatter and arc with a text control to show the rest. Proven by a spec that measures the editor width and went red on a 50/50 split. i64os main separately fails one command-palette unit test and its wiring register check, both untouched by this work; another session owns that.
- 2026-09-08 · B6 unparked and surveyed. Both sites share one database and one Better Auth version, but neither trusts the other's origin and a cookie cannot span two apex domains. Chosen: a one-time-token handoff (server-minted, single use, three minutes), carried by the existing cross-site links from your-pieces and the art site's account. Also found: the two sign-in configs drifted, and mandalacodes is the weaker one (raw OTP logged in dev, OTP not stored hashed, sessions not revoked on password reset); the build aligns it upward. Build dispatched. A migration, if the plugin needs one, lives only in Adrian-Website.
- 2026-09-08 · B6 built (mandalacodes PR 182 merged; Adrian-Website PR 135 merging on green). No migration needed: the token lives in the verification table both sites already share. Two-minute single-use token, hashed at rest, server-minted only, the return path allowlisted to same-origin paths. Mandalacodes sign-in raised to the art site's settings. **Needs Adrian once both are live:** sign in on one site, click the cross-site link, land signed in on the other. A cross-origin cookie flow cannot be walked by Playwright locally.
- 2026-09-08 · Launch-readiness site items 3 and 4 were already fixed on main (PR 165 on 2026-09-03 and commit d6477ae) before today's agents reached them; both agents found no live defect and landed only the missing regression proofs plus a Retry control on the offline notice (PRs 181, 183). Lesson filed as a working pattern: re-measure a plan's defect list against current main before dispatching on it. The launch-readiness "do first" list is now fully landed or superseded; treat it as history.
- 2026-09-08 · Mobile suite landed (PR 184, merged). The suite now starts its own server; measured before and after: 0 of 114 could run, now 70 pass, 9 skip, 35 fail. The job reports in CI without blocking until those 35 clear. 33 of the 35 trace to one cause: the July "one bottom bar" redesign force-hides the bottom nav class, and the admin voice recorder reuses that class, so it has been unreachable by touch since then. Two more are tests missing an atlas mock; three test a "Your codes" tile that is computed and never rendered. All filed in TODO.md under the mobile suite audit; the recorder is Adrian's call, the rest agent-runnable.

## Where this leaves things, end of 2026-09-08

Landed today: 16 pull requests across mandalacodes, Adrian-Website, and i64os. Every agent-runnable item in Tracks A, B, and C is done or superseded except the follow-ups in TODO.md. What remains is Adrian's: the pilot-card hour, the voice passes, the intention entrance, the atlas cuts, the two art-site switches, and one walk of the login handoff.
