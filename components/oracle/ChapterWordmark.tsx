    ...
    >
      ...
      >
        {chapters.map((chapter, idx) => {
          ...
          // Sticky band: Karla (sans), readable small, separated by thin
          // vertical rules. Inline strip: the larger editorial display serif
          ...
          const isFirst = idx === 0;
          // Each entry carries its OWN leading separator (a thin vertical rule
          // on the sticky band, a dot on the inline strip), inside the same
          // group as its label so the separator travels with the label and
          // never dangles at the start/end of a line.
          return (
            <span
              key={chapter.key}
              className="inline-flex items-center flex-shrink-0"
              style={sticky ? { columnGap: 'clamp(0.7rem, 3vw, 1.5rem)' } : { columnGap: 'clamp(0.5rem, 2.4vw, 1.75rem)' }}
            >
              {!isFirst && sticky && (
                <span aria-hidden="true" className={`block w-px h-3.5 ${paper ? 'bg-wood-400/45' : 'bg-stone-400/40'}`} />
              )}
              {!isFirst && !sticky && (
