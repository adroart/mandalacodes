...
- [ ] Deploy the hosted oracle Functions (`/api/oracle/{search,card,mcp}`) by merging to main; optionally set `ORACLE_MCP_TOKEN` to gate the remote MCP _(band: you-required)_ _(effort: quick)_ → Plan: [docs/oracle-remote-mcp-plan.md](docs/oracle-remote-mcp-plan.md)
