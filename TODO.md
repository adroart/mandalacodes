...
- [ ] Prove the oracle really works offline on your own phone _(band: you-required)_ _(effort: quick)_ → Plan: [offline-oracle-completion.md](todo/plans/offline-oracle-completion.md)
  The card words were proven offline; the artwork was only configured, never seen working, because the machine that built it could not reach the image host. Open the oracle on your phone, let a card load, wait ten seconds, switch to airplane mode, reload and open a card you never touched. Both words and art should appear. Nothing else in the offline plan matters until this passes.

- [ ] Finish the offline oracle: deploy checks, cache persistence, and one test that fails loudly _(band: agent-runnable)_ _(effort: moderate)_ → Plan: [offline-oracle-completion.md](todo/plans/offline-oracle-completion.md)
  A tab left open for days never notices a new release; the browser is never asked to keep the cache; and no test anywhere covers the offline path, so it can rot silently. Two of the plan's six items are your calls, not build work: whether the site should say out loud that it works without a connection, and what to do about iOS clearing its cache after a week unused (which matters if a gallery iPad is real).
