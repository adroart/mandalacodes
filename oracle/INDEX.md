...
**Start here for any card work.** [`CARD-PASS.md`](CARD-PASS.md) is the single entry
point: where the files sit, how one writer takes one card, what to run, and the handoff
for a fresh session. Under it sit the template ([`WRITERS-BRIEF.md`](WRITERS-BRIEF.md)),
the entrance's numbered tests ([`CARD-CHECKLIST.md`](CARD-CHECKLIST.md)) and the state of
the deck ([`CARD-PROGRESS.md`](CARD-PROGRESS.md), generated by `npm run progress`). The
entrance is part of the card template, not a separate job.
