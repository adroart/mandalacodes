...
**Start here for any card work.** [`CARD-PROGRESS.md`](CARD-PROGRESS.md) says which of
the sixty-four are written and which are not; it is generated by `npm run progress` off
the cards themselves and is the only place that state lives. [`ENTRANCE-DISPATCH.md`](ENTRANCE-DISPATCH.md)
says how one writer takes one card: what to read, which card to take so two sessions never
open the same file, the checks to run, and that there is no review round inside a pass.
[`ENTRANCE-CHECKLIST.md`](ENTRANCE-CHECKLIST.md) is the entrance's 49 rules, 28 of them run
by `npm run lint:entrance` and `npm run lint:slop`.
