 ...
 *
 *  1. Fetch artwork from production R2. tests/fixtures.ts answers every
 *     /media request with a 1x1 PNG; this proves the route is
...
test('a card page asks for a few images, and every one is answered locally', async ({ page }) => {
  const sizes: number[] = [];
