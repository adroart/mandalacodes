 ...
 *
 * Also carries the `oneTimeToken` plugin for the cross-site login handoff
 * with adrianrasmussen.com (identity direction, i64os/substrate/directions/
 * identity.md): both sites bind the SAME D1 database, so a token minted
 * here against this session is verifiable there against the same
 * `verification` row — no new table, no network call between the sites. See
 * functions/api/auth/handoff.js and functions/api/auth/handoff/accept.js.
 *
...
export function createAuth(env, waitUntil) {
  ...
  return betterAuth({
    ...
    // revokeSessionsOnPasswordReset matches the art site's stricter setting
    // (2026-09 drift alignment): a password reset ends every other session
    // on the shared user, not just the one that reset it.
    emailAndPassword: {
      enabled: true,
    ...
    plugins: [
      emailOTP({
        otpLength: 6,
        expiresIn: 10 * 60,
        // Hashed at rest, matching the art site (2026-09 drift alignment):
        // a raw sign-in code should never sit readable in the shared DB.
        storeOTP: 'hashed',
        async sendVerificationOTP({ email, otp }) {
          if (!env.RESEND_API_KEY) {
            // Fail loudly, matching the art site (2026-09 drift alignment):
            // a misconfigured mail key must break sign-in, not quietly log
            // the one-time code to the console.
      ...
      // Cross-site login handoff (mandalacodes.com <-> adrianrasmussen.com).
      // Single use (verifying consumes the `verification` row), short-lived,
      // hashed at rest, and mintable only from server code (never a raw
      // client HTTP request to /api/auth/one-time-token/generate) — see
      // functions/api/auth/handoff.js for the only caller.
      oneTimeToken({
        expiresIn: 2,
        storeToken: 'hashed',
