...
export function createAuth(env, waitUntil) {
  ...
  return betterAuth({
    ...
    // run, afterDelete mirrors the old Clerk `user.deleted` webhook cleanup:
    // remove the app `users` row (separate from Better Auth's `user` table)
    // and unbind any atlas steward records. Heavy atlas/db deps are imported
    // lazily so normal auth requests stay lean.
    user: {
      deleteUser: {
        ...
        afterDelete: async (deletedUser) => {
          ...
          try {
            const { deleteUserByClerkId } = await import(
              '../../functions/api/_lib/db.js'
            );
            await deleteUserByClerkId(env.DB, deletedUser.id);
          } catch {
            // best-effort; the Better Auth `user` row is already gone
          }
          try {
            const { unbindStewardsForUser } = await import(
              '../../functions/api/atlas/_helpers'
            );
            await unbindStewardsForUser(env, deletedUser.id);
          } catch {
            // best-effort steward unbind; never block the deletion
          }
        },
      },
    },
